...
export default function Projects( ) {
    ...
    return (<div>
        ...
        <main className='contact position-relative '>
            ...
            <section className='projects container position-relative img-projects'>
                <React.Fragment  >
                    ...
                    {dataPhoto.map((Element, index) => (
                        <React.Fragment key={index}>
                            <div className='col-sm-6 mb-4'>
                                <Link href={'/projects/' + Element.id} >
                                    <h1 className='text-main'>  {Element.category} </h1>
                                    <img src={'https://dashboard.st-supporteam.com/storage/app/' + Element.image} className='w-100 img-Project' alt={Element.title} title={Element.title} />
                                </Link>
                            </div>
                        </React.Fragment>
                    ))}
...
export async function getStaticProps() {
    const res = await fetch(`https://dashboard.st-supporteam.com/api/Portfolio`)
    const data = await res.json();
    const resCompany = await fetch(`https://dashboard.st-supporteam.com/api/companySetting`)
